...
class _DynamicsAgentCore(flax.struct.PyTreeNode):
    ...
    def infer_subgoal_for_eval(
        ...
        *,
        ...
        """Eval-only subgoal: sample ``N`` endpoints and optionally pick via critic.

        Training uses ``subgoal_num_samples``; eval uses ``subgoal_eval_num_samples``.
        When ``selection='best_of_n_value'`` and ``critic_agent`` is set, returns
        ``argmax_i V(s, z_i) V(z_i, g) / V(s, g)``.
        When ``selection='best_of_n_vzg'``, returns ``argmax_i V(z_i, g)``.
        ...
        if squeeze:
            obs = obs[None]
            goals = goals[None]
        num = max(1, int(num_candidates if num_candidates is not None else self.config.get('subgoal_eval_num_samples', 1)))
        if num <= 1:
            picked = self.infer_subgoal(obs, goals)
            if squeeze:
                picked = picked[0] if picked.ndim > 1 else picked
            return picked, rng

        if include_mean is None:
            include_mean = bool(self.config.get('subgoal_eval_include_zero_candidate', False))
        selection = str(
            selection if selection is not None else self.config.get('subgoal_eval_selection', 'best_of_n_value')
        ).lower()
        ...
        if critic_agent is None or selection not in ('best_of_n_value', 'best_of_n_vzg'):
            picked = mu
        else:
            params = critic_params if critic_params is not None else critic_agent.network.params
            if selection == 'best_of_n_vzg':
                scores = critic_agent.score_state_subgoals(
                    obs, candidate_goals, goals, network_params=params, energy_type='v_zg',
                )
            else:
                scores = critic_agent.score_transitive_subgoals(obs, candidate_goals, goals, network_params=params)
            best_idx = jnp.argmax(scores, axis=1)
            picked = candidate_goals[jnp.arange(obs.shape[0]), best_idx]
        if squeeze:
            picked = picked[0]
        return picked, rng
...
def _get_common_config():
    ...
    return ml_collections.ConfigDict(
        dict(
            ...
            # Eval-only best-of-N subgoal selection (decoupled from training
            # ``subgoal_num_samples``). Used by ``infer_subgoal_for_eval`` /
            # ``main._evaluate_env_tasks``; training loss still uses
            # ``subgoal_num_samples``.
            subgoal_eval_num_samples=1,
            # Eval best-of-N critic scoring: 'best_of_n_value' uses
            # V(s,z)V(z,g)/V(s,g); 'best_of_n_vzg' uses V(z,g) only.
            subgoal_eval_selection='best_of_n_value',
            subgoal_eval_include_zero_candidate=False,
            # Base seed for eval subgoal sampling; -1 uses ``eval_seed`` from the
            # eval loop. Per-episode key is ``base + ep_ix``; per-replan key adds
            # the replan index within the episode.
            subgoal_eval_seed=-1,
