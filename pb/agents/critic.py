...
from utils.trl_critic_config import strip_trl_unused_critic_keys
...
class CriticAgent(flax.struct.PyTreeNode):
    ...
    def _state_subgoal_score(
        ...
        if energy_type == 'v_product':
        ...
        elif energy_type == 'v_zg':
            v_z_g = jax.nn.sigmoid(self.network.select('value')(z_flat, g_flat, params=network_params))
            score = jnp.clip(v_z_g, 0.0, 1.0)
        elif energy_type == 'qz':
            ...
            raise ValueError(
                f"state_spi_energy_type must be 'v_product', 'v_zg', or 'qz', got {energy_type!r}"
            )
    ...
    def energy_state_subgoals(
        ...
        Option B (``v_zg``):        ``S = V(z, g)`` (subgoal-to-goal leg only).
        Option C (``qz``):        ``S = Q_Z(s, z; g)``.
        """
    ...
    def score_transitive_subgoals(
        ...
        """Transitive subgoal score ``V(s, z_i) V(z_i, g) / V(s, g)``, shape ``[B, N]``."""
        obs = jnp.asarray(observations, dtype=jnp.float32)
        z = jnp.asarray(subgoals, dtype=jnp.float32)
        if z.ndim == 2:
            z = z[:, None, :]
        ...
        if g.ndim == 2:
            ...
            raise ValueError(
                f'score_transitive_subgoals: goals shape {g.shape} incompatible with N={num}.'
            )
        obs_rep = jnp.repeat(obs[:, None, :], num, axis=1).reshape(bsz * num, -1)
        z_flat = z.reshape(bsz * num, -1)
        g_flat = g.reshape(bsz * num, -1)
        v_s_z = jax.nn.sigmoid(self.network.select('value')(obs_rep, z_flat, params=network_params))
        v_z_g = jax.nn.sigmoid(self.network.select('value')(z_flat, g_flat, params=network_params))
        v_s_g = jax.nn.sigmoid(self.network.select('value')(obs, goals, params=network_params))
        eps = jnp.asarray(float(self.config.get('subgoal_value_ratio_eps', 1e-3)), dtype=jnp.float32)
        score = v_s_z * v_z_g / jnp.clip(v_s_g[:, None], eps, 1.0)
        return jnp.clip(score.reshape(bsz, num), 0.0, 1.0)

    def value_subgoal_to_goal(
        self,
        subgoals: jnp.ndarray,
        goals: jnp.ndarray,
        network_params: dict | None = None,
    ) -> jnp.ndarray:
        """Probability-scale ``V(z, g)`` for candidate subgoals, shape ``[B, N]``.

        Used for the state-SPI anchor distribution ``w_m ∝ exp(beta * V(z_m, g))``.
        """
        z = jnp.asarray(subgoals, dtype=jnp.float32)
        if z.ndim == 2:
            z = z[:, None, :]
        ...
        if g.ndim == 2:
            ...
            raise ValueError(
                f'value_subgoal_to_goal: goals shape {g.shape} incompatible with N={num}.'
            )
        z_flat = z.reshape(bsz * num, -1)
        g_flat = g.reshape(bsz * num, -1)
        v_z_g = jax.nn.sigmoid(self.network.select('value')(z_flat, g_flat, params=network_params))
        return v_z_g.reshape(bsz, num)
